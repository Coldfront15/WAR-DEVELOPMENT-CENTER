...
private _veh = nearestObjects [player, [_shell], 3];
{
	if (typeOf _x == _shell) exitWith {
		private _dpicmTypes = [
			QUOTE(wdc_artillery_SPE_M3_M1_HE_CHARGE_1),
			QUOTE(wdc_artillery_SPE_M3_M1_HE_CHARGE_2),
			QUOTE(wdc_artillery_SPE_M3_M1_HE_CHARGE_3),
			QUOTE(wdc_artillery_SPE_M3_M1_HE_CHARGE_4),
			QUOTE(wdc_artillery_SPE_M3_M1_HE_CHARGE_5),
			QUOTE(wdc_artillery_SPE_M3_M60_WP_CHARGE_1),
			QUOTE(wdc_artillery_SPE_M3_M60_WP_CHARGE_2),
			QUOTE(wdc_artillery_SPE_M3_M60_WP_CHARGE_3),
			QUOTE(wdc_artillery_SPE_M3_M60_WP_CHARGE_4),
			QUOTE(wdc_artillery_SPE_M3_M60_WP_CHARGE_5)
		];

		if ((typeOf _x) in _dpicmTypes) then {
			private _timer = _x getVariable ["wdc_artillery_fuse_timer", 99999];
			_arty setVariable [QGVAR(FUSE_TIMER), _timer, true];
		};
